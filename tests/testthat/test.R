...
test_that("Some smoke tests", {
   ...
   juliaEval("String[]")
   expect(
      !is.null(juliaEval("using Random; Random.seed!(5)")),
      "Must be able to set random seed"
   )
   expect(
      is.null(juliaEval("Random.seed!(5);")),
...
test_that("No Revise message", {
   ...
   expect_false(grepl("Revise", as.character(output), fixed = TRUE))
})
...
test_that("read_bin fails cleanly if the stream ends prematurely", {
   expect_error(
      juliaEval(
         "RConnector.read_bin(RConnector.CommunicatoR(IOBuffer([0x01])), 4)"
      ),
...
test_that("Echo: 1-element vector of Int in Julia", {
   testEcho(juliaEval("[1]"))
})
test_that("Echo: Matrix of Int", {
...
test_that("Echo: 0-element vector of Float64 in Julia", {
   testEcho(juliaEval("Float64[]"))
})
test_that("Echo: 1-element vector of Float64 in Julia", {
   testEcho(juliaEval("[1.0]"))
})
...
test_that("Echo: 1-element vector of Float32 in Julia", {
   testEcho(juliaEval("[1.0f0]"))
})
test_that("Echo: matrix of Float32 in Julia", {
   testEcho(juliaEval("[1.0f0  2.0f0; 3.0f0 4.0f0]"))
})

test_that("Echo: 1-element vector of String in Julia", {
   testEcho(juliaEval('String["bla"]'))
})
test_that("Echo: 0-element vector of String in Julia", {
   testEcho("String[]")
})
test_that("Echo: 1-element vector of String in R", {
   testEcho("bla")
})
test_that("Echo: 2-element vector of String in R", {
   testEcho(c("bla", "blup"))
})
test_that("Echo: 2-element vector of String in Julia", {
   testEcho(juliaEval('String["bla", "blup"]'))
})
...
test_that("Echo: Int vector with missing values", {
   expect_true(juliaLet(
      "x[1] == 1 && ismissing(x[2]) && x[3] == 3",
      x = c(1L, NA, 3L)
   ))
...
test_that("Echo: Double vector with missing values", {
   ...
   x <- juliaEval(
      "Union{Float32,Missing}[1.0f0 missing 3.0f0; 3.0f0 missing 4.0f0 ]"
   )
   expect_true(all(is.na(x[, 2])))
   expect_equal(x[1, 1], 1)
   expect_equal(x[2, 1], 3)
...
test_that("NaN and NA are handled differently", {
   ...
   expect_equal(
      juliaCall("typeof", c(1, NA)),
      juliaEval("Array{Union{Float64,Missing},1}")
   )
   expect_equal(juliaCall("typeof", c(1, NaN)), juliaEval("Array{Float64,1}"))

   expect_equal(
      juliaCall("typeof", c(1 + 1i, NA)),
      juliaEval("Array{Union{Complex{Float64},Missing},1}")
   )
   expect_equal(
      juliaCall("typeof", c(1 + 1i, NaN)),
...
test_that("Complex with missing values", {
   juliaLet(
      "x[1] == 1+1im && x[2] == 2+2im && ismissing(x[3])",
      x = c(1 + 1i, 2 + 2i, NA)
   )
   x <- c(1 + 1i, 2 + 2i, NA)
   testEcho(x)
...
test_that("Complex values are handled first class", {
   ...
   for (complexPar in complexTypeParameters) {
      ...
      if (complexPar == "Float64") {
         ...
         expect_true(juliaCall(
            "==",
            juliaEval(attr(c, "JLTYPE")),
...
test_that("Echo: Single Int16", {
   testEcho(juliaEval('Int16(300)'))
})
test_that("Echo: Int16 Vector", {
   testEcho(juliaEval('Int16[1,2,3]'))
})
test_that("Echo: 1-element Int16 Vector", {
   expect_equal(juliaCall("string", juliaEval('Int16[300]')), "Int16[300]")
})


test_that("Echo: Single UInt128", {
   testEcho(juliaEval('UInt128(2)^100 +1'))
})
test_that("Echo: UInt128 Vector", {
   testEcho(juliaEval('UInt128[1,2,3]'))
})
test_that("Echo: 1-element UInt128 Vector", {
   expect_equal(
      juliaCall("string", juliaEval('UInt128[1]')),
...
test_that("Mutable struct usable by reference", {
   ...
   invisible(gc())
   juliaEval("1")
   expect_false(juliaLet(
      "haskey(communicator.sharedheap, ref)",
      ref = ref,
...
test_that("Arrays with undef entries are translated", {
   ...
   # all undefs
   testEcho(
      juliaEval("Vector{MutableTestStruct}(undef, 3)"),
      comparableInJulia = FALSE
   )

   # undefs and values
   juliaLet(
      "x = Vector{MutableTestStruct}(undef, 3);
   ...
   x <- juliaLet(
      "x = Array{MutableTestStruct}(undef, 2, 3, 4);
...
test_that("Let: used like eval", {
   output <- capture_output({
      expect(is.null(juliaLet("print(1)")), "Failed")
   })
   expect_equal(output, "1")
})
test_that("Let: must error with no named argument", {
   expect_error(juliaLet("print(x)", 1), "")
})
test_that("Let: basic echo", {
   expect(all(juliaLet("identity(x)", x = c(2, 3)) == c(2, 3)), "Failed")
})
...
test_that("Echo: Dictionary", {
   ...
   expect_equal(
      d[juliaExpr(":bla"), juliaExpr(":hello"), juliaExpr(":hi")],
      as.list(c(1, 17, 16))
   )
   expect_length(
      d[
...
test_that("Echo: Set", {
   ...
   expect_length(
      setdiff(juliaGet(juliaEval("Set([1; 2; 3; 4])")), c(1, 2, 3, 4)),
      0
   )
   expect_length(
      setdiff(juliaGet(juliaLet("setdiff(s1, s2)", s1 = s1, s2 = s2)), c(3, 4)),
      0
   )
   testEcho(s1)
})
...
test_that("Exotic objects handled gracefully", {
   ...
   expect_warning(
      {
         y <- juliaEcho(list(x = x))
      },
      regexp = "not translatable",
      all = FALSE
   )
   expect_null(y$x)
})
...
test_that("Complex Julia object with different member type", {
   juliaEval(
      'struct TestTypeWithAllKindsOfStuff
               n::Nothing
               f16::Float16
               f32::Float32
               f64::Float64
   ...
   juliaEval(
      'function TestTypeWithAllKindsOfStuff()
...
test_that("Imported modules are printed", {
   ...
   Pkg <- juliaImport("Pkg")
   expect_match(
      regexp = 'Julia module "Pkg": [0-9]+ function.*',
      capture.output(print(Pkg))[1]
   )

   # a user defined module
   module <- juliaEval(
      'module ImportTestModule1
               export f
               f(x)=1
             end'
   )
...
test_that("Errors are handled gracefully", {
   ...
   expect_error(juliaCall(
      "NotARealModule.thisisnotarealfunction",
      list(1, 2, 3)
   ))
})
...
test_that("Callback functions", {
   ...
   juliaEval(
      'function testNestedFun(ts::Vector{TestStruct}) map(t -> t.f(), ts) end'
   )
   t <- juliaCall(
      "testNestedFun",
      list(
         juliaCall("TestStruct", function() {
            doOutput("a")
         }),
         juliaCall("TestStruct", function() {
            doOutput("b")
   ...
   outputenv$output <- c()
   juliaEval(
      'function testNestedFunNamed(;ts::Vector{TestStruct}) map(t -> t.f(), ts) end'
   )
   t <- juliaCall(
      "testNestedFunNamed",
      ts = list(
   ...
   outputenv$output <- c()
   juliaEval(
      'function testNestedFun2(ts::Vector{TestStruct}) for i = 1:2 map(t -> t.f(), ts) end end'
   )
   t <- juliaCall(
      "testNestedFun2",
      list(
   ...
   outputenv$output <- c()
   juliaEval(
      'function testNestedFunArgs(ts::Vector{TestStruct}, args...) map(t -> t.f(args...), ts) end'
   )
   t <- juliaCall(
      "testNestedFunArgs",
      list(
         juliaCall("TestStruct", function(x, y, z) {
            doOutput(x)
         }),
         juliaCall("TestStruct", function(x, y, z) {
            doOutput(z)
   ...
   outputenv$output <- c()
   juliaEval(
      'function testNestedAndUnnested(f::Function, ts::Vector{TestStruct}, args...)
   ...
   t <- juliaCall(
      "testNestedAndUnnested",
      function(x, y) {
         doOutput(x)
      },
      list(
         juliaCall("TestStruct", function(x, y) {
            doOutput(x)
         }),
         juliaCall("TestStruct", function(x, y) {
            doOutput(y)
...
test_that("juliaCall can handle keyword arguments with key \"name\"", {
   ...
   expect_equal(
      Pkg$PackageSpec(name = "RData", version = "v0.10")$name,
      "RData"
   )
...
test_that("Constructors stand for their types", {
   ...
   expect_equal(
      as.character(juliaCall("typeof", TestTypeModule$TestType)),
      "DataType"
   )
})
...
test_that("Parametric types are imported", {
   juliaEval(
      "module ParametricTypeTestModule
...
test_that("Julia startup options can be modified", {
   ...
   expect_match(
      capture.output({
         juliaEval("import Pkg; Pkg.status()")
      }),
      regexp = "Dirname with Spaces",
      fixed = TRUE,
      all = FALSE
   )
   stopJulia()
   ...
   Sys.setenv(
      "JULIACONNECTOR_JULIAOPTS" = paste(specifyProjectOpts, "--depwarn=no")
   )
   juliaDepWarn <- 'Base.depwarn("there is a deprecation warning", :foo)'
   expect_length(
      capture.output({
         juliaEval(juliaDepWarn)
      }),
      0
   )
   stopJulia()
...
test_that("Error if Julia is not setup properly", {
   ...
   expect_error(
      JuliaConnectoR:::runJuliaServer(16752),
      "No Julia executable file found"
   )
   if (oldJuliaBindir == "") {
...
test_that("Fallback on juliaup fails correctly", {
   ...
   if (!file.exists(juliaCmdJuliaup)) {
      expect_error(
         JuliaConnectoR:::fallbackOnDefaultJuliaupPath(),
         regexp = "\\?`Julia-Setup`"
      )
   } else {
      expect_equal(
...
test_that("Circular references do not lead to a crash", {
   ...
   try(
      {
         juliaEval(definition)
      },
      silent = TRUE
   ) # (ignore redefinition error)
...
test_that("Test BigInt: a Julia object with external pointers", {
   ...
   expect_equal(
      juliaLet("Int(div(div(p,i1),i2))", p = p, i1 = i1, i2 = i2),
      2147483647
   )
   ...
   expect_true(juliaLet(
      "communicator.sharedheap[ref].refcount > 0",
      ref = i1Ref,
...
test_that("Serialized mutable struct can be restored", {
   juliaEval(
      "mutable struct TestSerializationStruct
               i::Int
             end"
   )
...
test_that("Operators can be imported and used", {
   ...
   suppressWarnings({
      base <- juliaImport("Base", all = FALSE)
   })
...
test_that("Display of default printing is limited to width", {
   ...
   y <- juliaPut(x)
   printOutput <- capture.output({
      print(y)
   })
   expect_lte(nchar(printOutput[length(printOutput)]), getOption("width"))
})


test_that("Error message for tables", {
   capture.output({
      JuliaConnectoR:::showUpdateTablesMsg()
   })
...
test_that("Data frame can be translated", {
   ...
   Pkg$activate(system.file(
      "examples",
      "IndexedTables-Project",
      subproject,
...
test_that("Attempt to translate Julia Table with incompatible types results in proper error", {
   ...
   expect_error(
      as.data.frame(x),
      regexp = 'Column type "Any" cannot be translated'
   )
})
...
test_that("Broadcasting via dot syntax works", {
   ...
   expect_equal(
      juliaGet(juliaCall("Statistics.mean.", arrofarrs, dims = 1L)),
   ...
   expect_equal(
      juliaCall("TestBroadcasting.my_add.", 10, c(1, 2, 3)),
      juliaEval("TestBroadcasting.my_add.(10, [1, 2, 3])")
   )
   expect_equal(
      juliaCall("TestBroadcasting.my_mul.", c(1, 2, 3), c(4, 5, 6)),
...
test_that("Iris/Flux example from README works", {
   ...
   suppressWarnings(eval(
      parse(text = paste(irisExampleRCode, collapse = "\n")),
      envir = scriptEnv
   ))
